...
//                hoaDonBLL.insertHoaDon(hoaDon.getMaSV());
...
//                System.out.println(rsHoaDon.getString(1));
//                hoaDon.setSoHoaDon(Integer.parseInt(rsHoaDon.getString(1)));
//                ThemGT=1;
//                jButtonXemHD.setEnabled(true);
//            }
//            ResultSet rs = chiTietHoaDonBLL.getChiTietHoaDon(hoaDon.getSoHoaDon(), giaoTrinhs.get(indexGT).getMaGT());
//            if (rs.next() == false) {
//                DTOChiTietHoaDon a = new DTOChiTietHoaDon();
//                a.setSoHD(hoaDon.getSoHoaDon());
//                a.setMaGT(giaoTrinhs.get(indexGT).getMaGT());
//                a.setSoLuongBan(Integer.parseInt(jTextFieldSoLuong.getText()));
//                chiTietHoaDonBLL.insertChiTietHoaDon(a.getSoHD(), a.getMaGT(), a.getSoLuongBan(), java.time.LocalDate.now().toString());
//                a.setNgayBan(java.time.LocalDate.now().toString());
//                chiTietHoaDons.add(a);
//                LoadBang();
//            } else {
//                //DTOChiTietHoaDon a = new DTOChiTietHoaDon();
//                chiTietHoaDonBLL.updateChiTietHoaDon(hoaDon.getSoHoaDon(), giaoTrinhs.get(indexGT).getMaGT(), Integer.parseInt(jTextFieldSoLuong.getText()), java.time.LocalDate.now().toString());
//                chiTietHoaDons.clear();
//                rsChiTietHoaDon = chiTietHoaDonBLL.getChiTietHoaDon(hoaDon.getSoHoaDon());
//                while (rsChiTietHoaDon.next()) {
//                    DTOChiTietHoaDon a = new DTOChiTietHoaDon();
//                    a.setSoHD(Integer.parseInt(rsChiTietHoaDon.getString("SoHoaDon")));
//                    a.setMaGT(Integer.parseInt(rsChiTietHoaDon.getString("MaGT")));
...
//                LoadBang();
//            }
//            ResultSet rsT = chiTietHoaDonBLL.TinhTien(hoaDon.getSoHoaDon());
//            rsT.next();
//            String tien = rsT.getString("TongTien") + " VNĐ";
//            jLabelThanhTien.setText(tien);
//        } catch (SQLException ex) {
//            Logger.getLogger(PanelQuanLyBanGiaoTrinh.class.getName()).log(Level.SEVERE, null, ex);
//        } catch (Exception ex) {
//            JOptionPane.showMessageDialog(jButtonDongY, ex.getMessage(), "Lỗi nhập liệu", HEIGHT);
//        }
        try {
            int indexGT = jComboBoxTenGT.getSelectedIndex();
            ResultSet testSL = giaoTrinhBLL.getGiaoTrinhByID(giaoTrinhs.get(indexGT).getMaGT());
            ...
            int slCo = Integer.parseInt(testSL.getString(8));
            if (slCo < Integer.parseInt(jTextFieldSoLuong.getText())) {
                throw new Exception("Không đủ số lượng sách để bán!");
            }
